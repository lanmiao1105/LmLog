...
public class ServiceAspect {
     ...
     * 生命周期切点
     */
    @Pointcut("execution(* android.app.Service.on*(..))")
    ...
    private void autoLog(ProceedingJoinPoint point, Throwable error, Object result, long lengthMillis) {
        ...
        String[] serviceFilters = LmLog.getConfig().getServiceFilters();
        if (serviceFilters != null) {
            for (int i = 0; i < serviceFilters.length; i++) {
                if (declaringTypeName.equals(serviceFilters[i])) {
                    return;
                }
            }
        }
